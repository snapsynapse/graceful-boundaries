...
`.github/workflows/test.yml` runs `npm test` (276 tests) and then the offline search contract on every push and pull request. The production contract is deliberately not wired into pull-request CI: it asserts against the deployed origin, which lags the branch under test.
