...
- `evals/test-*.js` — unit tests (276 tests across 13 files)
...
- `scripts/build-llms-full.js` — generates `llms-full.txt` from the spec sources (`npm run build:llms-full`)
- `.well-known/security.txt` — RFC 9116 security contact; refresh `Expires` before it lapses (tests fail after expiry)

## Commands

```bash
npm test                                          # run all 276 unit tests (13 files, no deps)
...
- Open questions live in `INTENT.md`. `llms.txt` is a navigation index; `llms-full.txt` is generated by `npm run build:llms-full` (never edit it by hand; a test fails on drift). Future and deferred spec ideas are in `docs/roadmap.md`. Repo Standards v0.4 recognizes the root-level "skill IS the repo" layout used here.
